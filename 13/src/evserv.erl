...
loop(S = #state{}) ->
    receive
        ...
        {Pid, MsgRef, {cancel, Name}} ->
            NewEvents = case orddict:find(Name, S#state.events) of
                            {ok, Event} ->
        ...
        {'DOWN', Ref, process, _Pid, _Reason} ->
            loop(S#state{clients=orddict:erace(Ref, S#state.clients)});
        code_change ->
            ?MODULE:loop(S);
        Unknown ->
...
valid_datetime({Date,Time}) ->
    try
        ...
        error:function_clause -> %% {{年,月,日}, {時,分,秒}} 形式でないとき
            false
    end;
